{ pkgs, config, ... }: {
  programs.neovim = {
    ...
    initLua = ''
      ...
      vim.schedule(function() require('config.recolor').apply() end)
    '';
    plugins =
  ...
  xdg.configFile."nvim/lua/stylix_palette.lua".text = ''
    return {
      ...
      overrides = {
        comment = '#${config.lib.stylix.colors.base04}',
        whitespace = '#${config.lib.stylix.colors.base0F}',
        delimiter = '#${config.lib.stylix.colors.base03}',
      },
    }
  '';
